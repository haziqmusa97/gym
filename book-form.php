...
$query="insert into  booking(sescode,time,feespm,protstatus,starton,duration,trainer,regno,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
$stmt->execute();
echo"<script>alert('Booking Successfully register');</script>";
}
?>
...
$uid=$_SESSION['login'];
				...
				if($rs)
				{ ?>
			<h3 style="color: red" align="left">Session already booked</h3>
				<?php }
				else{
							echo "";
							}			
...
<label class="col-sm-4 control-label"><h4 style="color: green" align="left">Session Related info </h4> </label>
</div>
...
{
?>
<option value="<?php echo $row->sescode;?>"> <?php echo $row->sescode;?></option>
<?php } ?>
</select> 
...
<div class="form-group">
<label class="col-sm-2 control-label">Protein Status</label>
<div class="col-sm-8">
<input type="radio" value="0" name="foodstatus" checked="checked"> Without Protein
<input type="radio" value="1" name="foodstatus"> With Protein (RM 200.00 )
</div>
</div>	
...
<div class="form-group">
<label class="col-sm-2 control-label">Personal Trainer </label>
<div class="col-sm-8">
<select name="course" id="course" class="form-control" required> 
<option value="">Select Trainer</option>
<?php $query ="SELECT * FROM trainer";
$stmt2 = $mysqli->prepare($query);
$stmt2->execute();
$res=$stmt2->get_result();
while($row=$res->fetch_object())
{
?>
<option value="<?php echo $row->fullname;?>"><?php echo $row->fullname;?>&nbsp;&nbsp;(<?php echo $row->username;?>)</option>
<?php } ?>
</select> </div>
</div>
...
<div class="form-group">
<label class="col-sm-2 control-label">Contact No (+60) : </label>
<div class="col-sm-8">
